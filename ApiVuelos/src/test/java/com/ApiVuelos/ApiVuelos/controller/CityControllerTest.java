...
public class CityControllerTest extends TestCase {
    ...
    public void addTest(){
        ...
        try {
            String [] code=this.city.getIataCode().split("-");
            ...
            verify(this.city,times(1)).setState(ct);
            verify(this.city,times(1)).setIataCode(code[0] + "-" + code[1] + "-" + code[2]);
    ...
    public void updateTest(){
        ...
        try {
            ...
            when(this.cityService.getById(this.st.getId())).thenReturn(this.city);

            City ste=this.cityService.getById(this.city.getId());
            assertNotNull(ste);
            when(city.getIataCode()).thenReturn(city.getIataCode());
            String [] code=this.city.getIataCode().split("-");
            ...
            when(this.stateService.getByAttributeType(code[0]+"-"+code[1]+"-"+code[2])).thenReturn(this.st);
            State ct=this.stateService.getByAttributeType(code[0]+"-"+code[1]+"-"+code[2]);
            assertEquals(ct,this.city);
            verify(this.city,times(1)).setState(ct);
            ...
            assertFalse(this.city.validateNullEmpty());
            when(this.cityService.newObject(this.city)).thenReturn(this.city);
            City airt=this.cityService.newObject(this.city);
            assertEquals(this.city,airt);
